    ...
    function () {
      ...
      _createClass(BuyerMatcheListingComponent, [{
            ...
            ref.forEach(function (item) {
              ...
              if (item.data().Lookingpostcode == _this6.stateService.listingBuyer.Lookingpostcode) {
                ...
                var _loop = function _loop(i) {
                  _this6.MatchesService.getUser(_this6.matchedProperties[i].UserId).subscribe(function (ref) {
                    ref.forEach(function (elements) {
                      if (_this6.matchedProperties[i].UserId == elements.data().uid) {
                        _this6.Names = elements.data().Name;
                        _this6.title = elements.data().title;
                      }
                    });
                  });
                };

                for (var i = 0; i < _this6.matchedProperties.length; i++) {
                  _loop(i);
                }
                ...
                var _loop2 = function _loop2(_i) {
                  _this6.MatchesService.getUser(_this6.unmatchedProperties[_i].detail.UserId).subscribe(function (dref) {
                    dref.forEach(function (element) {
                      if (_this6.unmatchedProperties[_i].detail.UserId == element.data().uid) {
                        _this6.unNamed = element.data().Name;
                        _this6.title = element.data().title;
                      }
                    });
                  });
                };

                for (var _i = 0; _i < _this6.unmatchedProperties.length; _i++) {
                  _loop2(_i);
                }
              }
            });
        ...
        value: function sortData(sort) {
          var data = this.matchedProperties.slice();
          ...
          this.matchedProperties = data.sort(function (a, b) {
            var isAsc = sort.direction === 'asc';

            switch (sort.active) {
              case 'search':
                return compare(a.searchra, b.name, isAsc);
